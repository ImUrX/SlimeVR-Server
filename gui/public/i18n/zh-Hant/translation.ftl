...
settings-osc-router-network-address-description = 設定用來發送資料到 VRChat 的地址。
settings-osc-router-network-address-placeholder = VRChat IP 位址
...
onboarding-enter_vr-back = 返回到追蹤器部位指定
...
onboarding-assign_trackers-title = 追蹤器部位指定
